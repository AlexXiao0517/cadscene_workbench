...
def build_render_report(*, inputs: Mapping[str, object], output_video: str | Path, stats: Mapping[str, object], warnings: Sequence[str]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- camera_calibration: {inputs.get('camera_calibration')}",
            f"- terrain_context: {inputs.get('terrain_context')}",
            f"- terrain_controls: {inputs.get('terrain_controls')}",
            "",
            "## 输出",
            "",
            ...
            f"- CAD 中文标注数量: {stats.get('cad_text_count', 0)}",
            f"- camera path 帧数: {stats.get('camera_path_frame_count', 0)}",
            f"- 缺帧策略: {stats.get('camera_interpolation', 'nearest_frame')}",
            f"- 输出分辨率预设: {stats.get('output_resolution', inputs.get('output_resolution'))}",
            f"- 相机标定: {stats.get('camera_calibration')}",
            f"- 高程模式/覆盖率: {stats.get('terrain_mode')} / {stats.get('terrain_coverage')}",
            f"- 高程文件指纹: {stats.get('terrain_source_fingerprints', [])}",
            "",
            "## Overlay 参数",
            "",
