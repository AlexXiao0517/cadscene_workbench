...
def test_restore_rebinds_current_analysis_from_snapshot_after_cad_replacement(
    tmp_path: Path,
) -> None:
    service, repositories, queue = _service(tmp_path)
    job_ids, revision = _complete_analysis(service, repositories, queue, tmp_path)
    before = repositories.project.load("p1")
    original_jobs = tuple(queue.get(job_id) for job_id in job_ids)
    _persist_jobs(repositories, "p1", original_jobs)
    replacement = tmp_path / "replacement-current.dxf"
    replacement.write_text("replacement", encoding="utf-8")
    repositories.project.update(
        "p1",
        expected_revision=before.revision,
