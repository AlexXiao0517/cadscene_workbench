...
| `srt_fixed_track_visual_pose` | Supported with guard | Adaptive sampled COLMAP sparse reconstruction, triangulation/BA, and RADIAL calibration recover visual rotation, which is registered to the SRT→CAD base positions. The sparse cloud is diagnostic; six-DoF keyframe residual fitting can correct position and rotation without turning SfM centers into the base route. |
...
2. Open Project Clip Management and review the detected motion mode, SRT coverage, and recommended workflow. The selector supports SfM, OpenGV, and both SRT routes. Normally keep the recommendation and override it when site knowledge requires it. Batch trajectory solving and rendering require at least one checked clip; individual clips can still be opened in the workbench.
3. For either SRT route, enter the horizontal FOV and current CAD CGCS2000 central meridian, inspect the candidate-projection CAD bbox and SRT-path preview, and explicitly confirm the coordinate configuration.
4. For an `sfm_only` clip, run SfM, save at least two valid manual keyframes, fit the route, generate the keyframe plan, finish calibration, and run the final fit. `srt_full_pose` consumes confirmed metric positions and DJI gimbal poses directly and skips SfM. Missing-pose `srt_fixed_track_visual_pose` performs adaptive sampled COLMAP sparse reconstruction and calibration, then registers visual rotation to SRT positions. Its sparse cloud is diagnostic and optional downstream. Both SRT workbenches support branch-validated XYZ/rotation/FOV edits, route fitting, and a separate render action.
5. Run quality inspection for `sfm_only`; after it passes, choose Complete Quality Inspection to enter Render and Export, or return to keyframe calibration if more frames are required. Both SRT routes skip quality and proceed to fine-tune/render.
6. In Render and Export, either create CAD-anchored engineering callouts or render without callouts. Render Video binds the current annotation state into the immutable render revision.
7. Return to Project Clip Management. When every delivery clip has a current render, run Merge Output. Concatenation uses each `render_frame_map.json` source-PTS partition and rejects duplicate or missing frames.
8. After a refresh or service restart, use the same `--storage-root` and `project_id`. Saved workbench outputs, annotations, jobs, renders, and version history are restored from project manifests.
...
- The SRT routes solve horizontal projection, not the CAD elevation datum. Full pose uses `cad_z_offset_m`; fixed track applies `route_offset_xyz_m` to its Base route and can then fit six-DoF manual-key residuals into a Corrected route. Unverified absolute height remains warning evidence.
...
- Project-owned code is provided under the [MIT License](LICENSE); consult [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) before redistribution.
- Public v0.1.5 waits for third-party redistribution materials to be verified. Use the [Release page](https://github.com/AlexXiao0517/cadscene_workbench/releases/tag/v0.1.5) as the authority for actual assets. `CADScene-0.1.5.zip` and its checksum are planned names, not a claim that they are downloadable now. A CAD central meridian such as 118°50′ is separate from the integer horizontal FOV.
- Windows workspace upgrades are supported only on the same computer, not as cross-machine migration.
