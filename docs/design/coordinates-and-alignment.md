...
`pure_rotation` 工作流同样是 Experimental：相机中心固定，不恢复平移或尺度。项目视频分析会根据短视频中持续、无矛盾的旋转证据自动推荐该路线，推荐仍需结合现场确认固定相机中心；证据不足时保守推荐 `sfm_only`，项目页也允许人工覆盖。它依赖外部 OpenGV 后端；即使能执行，结果也不适用于推断沿路线行进距离。
