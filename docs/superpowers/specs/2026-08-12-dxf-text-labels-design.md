...
根据首轮浏览器验收反馈，CAD 文字在 3D 视图与视频叠加视图中统一放大约 20%。3D 标签通过独立的视觉缩放常量放大平面宽高，不改变 DXF 原始 `cad_height` 数据；视频叠加的普通标注基准字号由 13 px 调整为 16 px，桩号/大字级由 18 px 调整为 22 px。低质量拖动预览继续沿用相同比例缩放。

此次调整不增加活动标签数量、投影候选数量、空间网格规模、刷新频率或纹理缓存容量，因此交互性能边界保持不变。TDD 契约需同时验证 3D 缩放常量、两档视频字号和既有性能上限。
...
- 3D 与视频叠加文字相对首轮版本统一放大约 20%，DXF 原始字高数据保持不变。
